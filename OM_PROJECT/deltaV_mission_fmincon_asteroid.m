function [MISSION, x_opt, fval, exitflag, output] = deltaV_mission_fmincon_asteroid(MISSION, x0)
% DELTAV_MISSION_FMINCON_ASTEROID Continuous optimization of the minimum total DeltaV between two time windows
% using fmincon, with constraints on t2 > t1 and ||deltaV1|| <= V_launcher.
% This function is the analogue of deltaV_mission_fmincon but in the case
% of arrival to an asteroid, using the function ephAsteroids
%
...
[MISSION.MostEfficient.Eph_target_asteroid,~] = ephAsteroids(MISSION.MostEfficient.t2, MISSION.Data.target_asteroid);
...
function [DV_tot, DV1_norm, DV2_norm, TOF_s, v_t1_tr, v_t2_tr, v_t1_pl, v_t2_pl] = evaluateTransfer(t1_eval, t2_eval, M)
    ...
    [eph2,~] = ephAsteroids(t2_eval, M.Data.target_asteroid);
