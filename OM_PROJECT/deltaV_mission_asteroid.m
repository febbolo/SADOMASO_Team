function [MISSION] = deltaV_mission_asteroid(MISSION,time_step_case)
%DELTAV_MISSION This function is analogue to the deltaV_mission function,
% but in case of asteroid, using the function ephAsteroids. The Injection Orbit from departure planet and
% Insertion Orbit to arrival asteroid ARE NOT CONSIDERED.
%
...
for ii = 1:length(t1_v)
    ...
    for jj = 1:length(t2_v)
        ...
        % time t2
        [eph2, ~, ~] = ephAsteroids(t2_v(jj), MISSION.Data.target_asteroid);
        % Converting radiants in degrees
...
[MISSION.MostEfficient.Eph_target_asteroid,~] = ephAsteroids(MISSION.MostEfficient.t2,...
                                                   MISSION.Data.target_asteroid);
